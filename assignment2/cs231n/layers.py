...
def affine_backward(dout, cache):
  ...
  # compute dx = d Loss / d x
  dxin = dout.dot(w.T) # (N, D) = (N, M) dot (M, D)
  dx = np.reshape(dxin, x.shape)

  # compute dw = d Loss / d w
  N = x.shape[0]
  xin = np.reshape(x, (N, -1))
  dw = dout[:, np.newaxis, :] * xin[:, :, np.newaxis] # (N, D, M) = (N, 1, M) * (N, D, 1)
  dw = np.sum(dw, axis=0) # (D, M)

  # compute db = d Loss / d b
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    normalized = (x - mean) / np.sqrt(var + eps)  # N x D
    y = gamma * normalized + beta  # N x D
    out = y
    ...
    # the out variable.                                                         #
    #############################################################################
    normalized = (x - running_mean) / np.sqrt(running_var + eps)  # N x D
    y = gamma * normalized + beta  # N x D
    out = y
...
def batchnorm_backward(dout, cache):
  ...
  dnorm = dout * gamma # N x D

  dnorm_dx    =   1 / np.sqrt(var + eps) # D
  dnorm_dmean = - 1 / np.sqrt(var + eps) # D

  dvar = np.sum(-1./2 * (x - mean) / np.power(var + eps, 3./2) * dnorm, axis=0) # D
  dvar_dmean = -2/N * np.sum(x - mean, axis=0) # D

  dmean = np.sum(dnorm * dnorm_dmean, axis=0) + dvar * dvar_dmean # D

  dvar_dx = 2./N * (x - mean) # D
  dx = 1./N * dmean + dvar_dx * dvar + dnorm_dx * dnorm
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = (1. / N) / np.sqrt(var + eps)  * (N * dnorm - np.sum(dnorm, axis=0) - normalized * np.sum(dnorm * normalized, axis=0))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    # x: N x D
    mask = np.random.random(x.shape[1:]) # D
    mask[mask <= p] = 0
    mask[mask > p] = 1
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  # (N, C, H, W) swap -> (N, W, H, C) reshape -> (N*W*H, C)
  reshaped_x = x.swapaxes(1, 3).reshape(-1, C)

  reshaped_out, cache = batchnorm_forward(reshaped_x, gamma, beta, bn_param)

  # (N*W*H, C) reshape -> (N, W, H, C) swap -> (N, C, H, W)
  out = reshaped_out.reshape(N, W, H, C).swapaxes(1, 3)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  # (N, C, H, W) swap -> (N, W, H, C) reshape -> (N*W*H, C)
  reshaped_dout = dout.swapaxes(1, 3).reshape(-1, C)

  reshaped_dx, dgamma, dbeta = batchnorm_backward(reshaped_dout, cache)

  # (N*W*H, C) reshape -> (N, W, H, C) swap -> (N, C, H, W)
  dx = reshaped_dx.reshape(N, W, H, C).swapaxes(1, 3)
