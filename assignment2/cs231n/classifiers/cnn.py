...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    drelu_out, dW3, db3 = affine_backward(dout, affine_cache)
    dW3 += 0.5 * 2 * self.reg * W3

    dpool_out, dW2, db2 = affine_relu_backward(drelu_out, relu_cache)
    dW2 += 0.5 * 2 * self.reg * W2

    dx, dW1, db1 = conv_relu_pool_backward(dpool_out, pool_cache)
